...
/*
    public function testNullConstraints()
    {
        ...
        $table
            ->addColumn("a", "integer")
            ->addColumn("b", "integer", ['null' => true])
            ->addColumn("c", "integer", ['null' => false])
            ->save();

        $expectedOutput = <<<'OUTPUT'
CREATE TABLE `table1` (`a` INTEGER NOT NULL, `b` INTEGER NULL, `c` INTEGER NOT NULL);
OUTPUT;
